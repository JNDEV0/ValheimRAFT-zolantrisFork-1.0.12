...
public static class VehicleRecallController
{
  ...
  public static void TeleportPlayerToVehicle(Player player, int vehicleId)
  {
    ...
    // Vanilla distantTeleport triggers sector loading if distant
    player.TeleportTo(landingPos, targetRot, distantTeleport: !isLoaded);
    player.Message(MessageHud.MessageType.Center, $"Teleported to Vessel #{vehicleId}!");
  }
  ...
  public static void RecallVehicleToCrosshair(Player player, int vehicleId)
  {
    ...
    {
      targetPos = player.transform.position + rayDir * 25f;
    }

    var waterLevel = ZoneSystem.instance != null ? ZoneSystem.instance.m_waterLevel : 30f;
    targetPos = new Vector3(targetPos.x, Mathf.Max(targetPos.y, waterLevel), targetPos.z);
    ...
    if (isLoaded && vm != null)
    {
      ...
      {
        var zdo = vm.m_nview.GetZDO();
        var oldSec = zdo.GetSectorIndex();
        zdo.SetPosition(targetPos);
        zdo.SetRotation(targetRot);
        var newSec = zdo.GetSectorIndex();
        if (oldSec != newSec && ZDOMan.instance != null)
        {
          ZDOMan.instance.RemoveFromSector(zdo, oldSec);
          ZDOMan.instance.AddToSector(zdo, newSec);
        }
      }
    ...
    player.Message(MessageHud.MessageType.Center, $"Vessel #{vehicleId} recalled successfully!");
  }
  ...
  public static void AttunePlayerToVehicle(Player player, int vehicleId)
  {
    ...
    player.Message(MessageHud.MessageType.Center, $"Horn attuned to Vessel #{vehicleId}!");
  }

...
//     }
...

    {
      ...
      {
        _flightTakeoffImmunityTimer -= deltaTime;
      }
      else if (IsFlightModeActive && TargetHeight <= 0.05f && ZoneSystem.instance != null)
      {
...

      {
...

        {
          ...
          zsyncTransform.CustomFixedUpdate(deltaTime);
...

      {
...

        {
...

          {
            ...
            if (IsFlightModeActive && _flightTakeoffImmunityTimer <= 0f)
...
// Updates gravity and target height (which is used to compute gravity)
...

    {
      ...
      if (PropulsionConfig.AllowFlight.Value && IsFlightModeActive)
...

    {
      ...
      bool isCtrlPressed = ZInput.GetButton("Crouch") || ZInput.GetButton("JoyCrouch") || Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.RightControl);

      bool isSpacePressed = ZInput.GetButton("Jump") || ZInput.GetButton("JoyJump") || Input.GetKey(KeyCode.Space);

      bool isCtrlDown = ZInput.GetButtonDown("Crouch") || ZInput.GetButtonDown("JoyCrouch") || Input.GetKeyDown(KeyCode.LeftControl) || Input.GetKeyDown(KeyCode.RightControl);

      bool isSpaceDown = ZInput.GetButtonDown("Jump") || ZInput.GetButtonDown("JoyJump") || Input.GetKeyDown(KeyCode.Space);



      bool isToggleChord = (isCtrlPressed && isSpaceDown) || (isSpacePressed && isCtrlDown);
...

      {
        ...
        SetFlightMode(!IsFlightModeActive);
      ...
      var isAscendKeyPressed = GetAscendKeyPress && !isCtrlPressed;

      var isDescendKeyPressed = GetDescendKeyPress && !isSpacePressed;
 ...
 */
...

    {
      ...
      if (ZNetView.m_forceDisableInit || PiecesController == null) return;
...

      {
        ...
        zsyncTransform.CustomLateUpdate(Time.deltaTime);
    ...
    public bool IsFlightModeActive => m_nview?.GetZDO()?.GetBool(VehicleZdoVars.VehicleFlightMode, false) ?? false;



    public void ShowWheelHoverMessage(string text)

    {

      var rawMsg = Localization.instance != null ? Localization.instance.Localize(text) : text;

      if (string.IsNullOrEmpty(rawMsg) || rawMsg.StartsWith("$")) rawMsg = text;
    ...
    private float _flightTakeoffImmunityTimer = 0f;

    public void SetFlightMode(bool active, bool showMessage = true)

    {

      if (m_nview == null || m_nview.GetZDO() == null) return;

      var current = IsFlightModeActive;

      if (current == active) return;



      if (active && !PropulsionConfig.AllowFlight.Value) return;



      m_nview.GetZDO().Set(VehicleZdoVars.VehicleFlightMode, active);
...

      {
        ...
        ShowWheelHoverMessage(active ? "$valheim_vehicles_mode_flight" : "$valheim_vehicles_mode_float");
      ...
      {
        _flightTakeoffImmunityTimer = 0f;
      }
...

      {
        ...
        if (ZoneSystem.instance != null && TargetHeight > GetSurfaceOffsetWaterVehicleOnly())
      ...
      MapPinSync.Instance?.UpdatePinIconForZdo(m_nview.GetZDO(), active);
...

    {
      ...
      ShowWheelHoverMessage("$valheim_vehicles_raise_anchor_first");
